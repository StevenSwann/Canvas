...
namespace CanvasMVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(UserLoginDTO model, string returnUrl)
        {
            ...
            UserLoginDTO logUser = new UserLoginDTO();
                        
            logUser.EmailAddress = model.EmailAddress;
            logUser.Password = model.Password;            

            var content = JsonConvert.SerializeObject(logUser);
            var buffer = System.Text.Encoding.UTF8.GetBytes(content);
            var byteContent = new ByteArrayContent(buffer);
            byteContent.Headers.ContentType = new MediaTypeHeaderValue("application/json");

            HttpResponseMessage result = await client.PostAsync(apiUrl + "/LoginUser", byteContent);
            ...
            if (result.StatusCode == HttpStatusCode.OK)
            {
                ...
                var loggedUser = JsonConvert.DeserializeObject<AuthenticatedUserDTO>(responseData);
        ...
        public async Task<ActionResult> Register(UserRegisterDTO model)
        {
            if (ModelState.IsValid)
            {
                ...
                HttpResponseMessage result = await client.PostAsync(apiUrl + "/RegisterUser", byteContent);                              
